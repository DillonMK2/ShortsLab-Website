...
const socialLinks = [
  ...
  {
    href: 'https://discord.gg/uDGEMGRuqQ',
    label: 'Discord',
...
export default function Footer() {
  return (
    <footer className="relative border-t border-white/[0.05] bg-background">
      ...
      <div className="absolute top-0 left-1/2 -translate-x-1/2 w-1/2 h-px bg-gradient-to-r from-transparent via-accent-violet/50 to-transparent" aria-hidden="true" />
      ...
      <div className="max-w-7xl mx-auto px-6 py-12">
        <div className="flex flex-col md:flex-row items-center justify-between gap-8">
          ...
          <div className="flex items-center gap-6">
            {footerLinks.map((link) => (
              <Link
                key={link.href}
                href={link.href}
                className="relative text-white/50 hover:text-white transition-colors duration-200 text-sm hover-underline"
              >
                {link.label}
              </Link>
            ))}
          ...
          {/* Social Links - CSS hover effects instead of Framer Motion */}
          <div className="flex items-center gap-3">
            {socialLinks.map((social) => (
              <a
                ...
                className="w-9 h-9 rounded-full bg-white/[0.05] border border-white/[0.08] flex items-center justify-center text-white/50 hover:text-white hover:bg-white/[0.1] hover:border-accent-violet/50 transition-all duration-200 hover:scale-110 hover:-translate-y-0.5"
